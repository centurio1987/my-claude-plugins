...
1. **자기완결 inline React.** React Flow·Mermaid·차트 라이브러리·아이콘 패키지를 쓰지
   ...
   flexbox로만 그려서 어떤 MDX 호스트에서도 설정 없이 렌더된다. 아이콘은 유니코드
   글리프(`▼ → ↓ ✅ ⚠️ 💡 🔁`). 디자인 시스템을 바인딩해도 이 규칙은 안 바뀐다 —
   바뀌는 건 토큰 값과 컴포넌트 이름뿐이다.
...
- 사용자가 줄 수 있는 자료(디자인 토큰/theme 파일, repo 컴포넌트 라이브러리, Figma/
  스크린샷, 서술형 스펙, 디자인 시스템 repo URL)를 가볍게 묻되 **default도 좋은 선택지**로
  제시한다. 자료가 없거나 원치 않으면 `assets/theme.default.ts`를 그대로 profile의
  `theme.ts`로 복사하고 진행한다.
...
아키타입을 고른다: `BeforeAfter`·`LayerStack`·`FlowSteps`·`Architecture`·`CardSet`·
`Matrix`. 그다음:
- MDX 최상단에 profile theme을 `export const T = {...}`로 직렬화(쓰는 색 패밀리만).
...
- **렌더 가능성 lint를 반드시 통과시킨다:**
  ...
  본문 태그를 잡는다. 위반이 나오면 해당 대문자 컴포넌트를 소문자 헬퍼 함수 호출
  (`{panel(a)}`)이나 inline 소문자 HTML로 바꿔 **lint가 깨끗해질 때까지** 고친다.
